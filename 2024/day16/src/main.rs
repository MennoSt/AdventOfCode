...
fn iterate_maze(
    grid: &mut Grid,
    score_grid: &mut Gridi32,
    shortest_paths: &mut HashSet<Coordinate>,
    lowest_score: i32,
) {
...
fn next_it(
    ...
    let movements = vec![
        ...
        Movement {
            coordinate: Coordinate { x: x, y: y - 1 },
    ...
    for movement in movements {
        ...
        if grid_elem == "." || grid_elem == "E" {
            ...
            let score_increase =
                calculate_score_increase(&current_pos.direction, &movement.direction);
...
fn update_shortest_path(
    grid: &mut Grid,
    score: &mut i32,
    lowest_score_coord: &mut HashSet<Coordinate>,
    lowest_score: i32,
    grid_elem: String,
) {
...
fn solve(input: &str, lowest_score: i32) -> (i32, i32) {
    ...
    iterate_maze(
        &mut grid,
        &mut score_grid,
        &mut lowest_score_coord,
        lowest_score,
    );

    (
        score_grid._elem(end.x, end.y),
        lowest_score_coord.len() as i32,
    )
}
