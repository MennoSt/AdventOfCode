...
fn is_only_increase_or_decrease(input: &Vec<i32>, tolerate_single_bad: bool) -> bool {
    ...
    if ((increased && decreased) || (!increased && !decreased)) && tolerate_single_bad {
        for i in 1..input.len() {
            ...
            if is_only_increase_or_decrease(&vec_copy1, false)
                && has_correct_difference(&vec_copy1, false)
                || is_only_increase_or_decrease(&vec_copy2, false)
                    && has_correct_difference(&vec_copy2, false)
            {
                return true;
            }
        }
    ...
    for i in 1..input.len() {
        ...
        if diff > 3 || diff < 1 {
            if tolerate_single_bad {
                ...
                if has_correct_difference(&vec_copy1, false)
                    && is_only_increase_or_decrease(&vec_copy1, false)
                    || has_correct_difference(&vec_copy2, false)
                        && is_only_increase_or_decrease(&vec_copy2, false)
                {
                    return true;
